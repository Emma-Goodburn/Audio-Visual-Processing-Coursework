...
def main():
    ...
    directory = "C:/Users/thoma/Documents/GitHub/AVCoursework1/performanceTestRecordings"
    files = Path(directory).glob("*")
    folderName = "performanceTestMFCCs"
    for filename in files:
        signal, sampleRate = sf.read("performanceTestRecordings/" + filename.name, dtype='float32')
        name = filename.name.replace(".wav", "")
        featureExtraction.main(signal, sampleRate, name, folderName)
    
    
        directory = "C:/Users/thoma/Documents/GitHub/AVCoursework1/performanceTestMFCCs"
        files = Path(directory).glob("*") 
        folderName = "performanceTestMFCCs"
        for filename in files:
            # Load the saved MFCC
            mfcc = np.load(f"{folderName}/{filename.name}")
